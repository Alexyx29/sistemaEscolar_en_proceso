...
import iconoTask from '@/assets/icons/iconoTask.svg';
import selectIcon from '@/assets/icons/select-icon.svg';
import React, { useState } from 'react';


const ViewStatusExamStu = ({
    ...
    return(
        <>
            <div class={'w-[full] flex bg-[#F7F8F9] rounded-[10px] shadow-2xl relative z-10 '}>
                <div className={'flex w-full items-center my-[15px] ml-[40px]'}>
                    <img src={iconoTask.src} className='text-[#364C9B]' ></img>
            ...
            {content && (
            <div class={'w-[100%] flex bg-input-white rounded-[10px] mt-[-30px] '}>
                <div className='flex flex-col font-open text-[11px] font-semibold text-[#818181]  my-[15px] ml-[40px]' >
                    <label className={`cursor-pointer changeItem ${selectedFilter === 'Todos' ? 'text-[#364C9B]' : 'text-[#818181]'}`} onClick={() => handleFilterClick('Todos')}>Todos</label>
                    <label className={`cursor-pointer changeItem ${selectedFilter === 'Pendientes' ? 'text-[#364C9B]' : 'text-[#818181]'}`} onClick={() => handleFilterClick('Pendientes')}>Pendientes</label>
                    <label className={`cursor-pointer changeItem ${selectedFilter === 'Entregados' ? 'text-[#364C9B]' : 'text-[#818181]'}`} onClick={() => handleFilterClick('Entregados')}>Entregados</label>
...
export default ViewStatusExamStu;