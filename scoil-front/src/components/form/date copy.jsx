...
const Date = ({ label, hiddenMargin, id, name, type, placeholder = "", error = "", value = '', change = () => {}, disabled = false, maxDate = null, minDate = null, clases = 'rounded' }) => {
    ...
    return (
        <>
            <div className={'w-full'}>
                { label !== '' && 
                    <label className="block ml-1 overflow-hidden text-sm opensans text-gray_alter whitespace-nowrap text-ellipsis">{label}</label>
                }
                <div className={(hiddenMargin ? '' : 'md:mb-7 mb-2') + ' relative'}>
                    <input name={`${name}_text`} type="text" autoComplete="off" className={`w-full border ${clases} border-libellum-border focus:outline-none text-gray_alter placeholder:text-sm placeholder:opensans placeholder:text-gray_alter text-sm px-[10px] py-3 font-open h-9 dark:bg-input-dark dark:text-placeholder-dark dark:placeholder:text-placeholder-dark`} value={dateShow || placeholder} onChange={e => {setDateShow(e.target.value)}} onBlur={validateDateManual} />
                    ...
                    <input id={id} name={name} type={type} autoComplete="off" className="absolute overflow-hidden z-20 w-5 h-8 top-[2px] right-2 focus:outline-none opacity-0" onChange={e => {changeDate(e);}} ref={inputDate} disabled={disabled} {...((maxDate) ? {max: maxDate} : {})} {...((minDate) ? {min: minDate} : {})}/>
                    { errorLocal != '' && (
