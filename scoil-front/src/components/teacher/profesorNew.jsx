...
import infoLarge from '@/assets/background/info.jpg';
import CloseEffect from '@/components/modal/btns/closeEffect';
import DeleteEffect from '@/components/modal/btns/deleteEffect';
import SaveEffect from '@/components/modal/btns/saveEffect';
import EditEffect from '@/components/modal/btns/editEffect';
import ImageBase from '../items/imageBase';


const ProfesorNew = ({themeImg, userImg, name, role, time, title, content, icon, edit=false, lastname}) =>{
        ...
        <>
            <div className='flex  '>
                <div className='flex-flex-col'>                
                    ...
                    <div className={ themeImg ?  "bg-input-white rounded-b-lg" : 'bg-input-white rounded-[15px]'}>                
                        <div className='flex w-full justify-center'>
                            <div className='w-[95%] flex justify-between mt-[15px]'>
                                <div className='flex'>
                                    <div>
                                        <ImageBase image={userImg.src} size='60px' name={name} lastname={lastname} ></ImageBase>
                                    ...
                                    <div className='flex flex-col h-full justify-center ml-[7px]'>
                                        <div className='flex' >
                                            <label className='font-open text-[16px] font-bold text-news-name leading-none cursor-pointer mr-1'>{name}</label>
                                            <label className='font-open text-[16px] font-bold text-news-name leading-none cursor-pointer'>{lastname}</label>
                                        </div>
                                    ...
                                    {icon && (
                                        <div className='flex items-center mr-[10px]' onMouseOver={handleMouseOver} onMouseOut={handleMouseOut} style={ hovered ? {filter: 'invert(95%) sepia(79%) saturate(3323%) hue-rotate(207deg) brightness(90%) contrast(87%)'} : {filter: 'none'}}>
                                            <Link href={'/calendario-padres-eventos'}>
                                                <img src={icon} className='w-[50px] h-[40px] cursor-pointer'></img>
                                            </Link>                                        
                                        </div>
                                    )}
                ...
                {edit && (
                        <>
                            <div className="md:flex bg-form-btns text-white right-0  h-full hidden items-start justify-center w-[52px] rounded-tr-2xl rounded-br-2xl">
                                <div className="grid h-fit gap-y-6 flex-wrap max-h-[250px] items-start pt-[18px] flex-col">                                    
                                    <EditEffect color="hover:bg-btn-blue"></EditEffect>
                                    <DeleteEffect color="hover:bg-red-date"></DeleteEffect>                                                                        
                                </div>
                            </div>                    
                        </>
                )}
