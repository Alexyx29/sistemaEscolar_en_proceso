 ...
 * @returns 
 */
const BarraMenu = ({hidden, text, modules, hiddenMenu, user, space, urlImage, locale, spaceUsed, idiomas, system, clickBuy = () => {}, bag = [], bagCurrency = '', bagAmount= 0, removeItem = () => {}, paymentProcess = () => {}, showBusiness = () => {}, signOff = () => {}, botonSeleccionado = 'tablero' }) => {
    ...
    return (
        <>
            <aside className="relative min-h-[760px] min-w-[auto] bg-gray-light p-0 2xl:min-w-[290px] xl:w-[260px] rounded-[10px] mr-[30px] h-[94vh] ">
                <div className='flex flex-col'>
                    {/* h-[100%] */}
                    <div>
                        <Alumno name={user.name} lastname={user.lastName} carrera={user.carrera} curricula={user.curricula} tamanio='w-[100px] h-[100px]' imagen={user.image} borde={false} notificaciones= {user.notifications} reference='/datos-alumno' />
                    </div>
                            ...
                            {modules.map((mod, index) => {
                                return (
                                    <div key={index}>
                                        {mod.link ?
                                            <Link href={mod.link} >
                                                <ButtonIcon colors= {botonSeleccionado == mod.name ? '  bg-[#0082C9]   text-input-white' : 'bg-gray-light text-gray-menu-text'} txt_button={mod.txt_button} icono={mod.icono} filtro= {botonSeleccionado == mod.name  ? true : false } />
                                            </Link>
                                        :
